...
static esp_err_t start_dma(int line_width, int samples_per_cc, int ch = 1) {
...
#if defined(ENABLE_GPIO26)  // DAC, video on GPIO26
  I2S0.conf.tx_right_first = 0;
  dac_output_enable(DAC_CHANNEL_2);
  dac_output_disable(DAC_CHANNEL_1);
#else  // DAC, video on GPIO25
  ...
  return esp_intr_enable(_isr_handle);  // start interrupts!
}
...
void video_init_hw(int line_width, int samples_per_cc) {
  ...
  // nasty digital spikes and dropouts.
}
 ...
 */
void ESP_8_BIT_composite::begin(bool isDoubleBuffer) {
  instance_check();
